...
export function ResourceEditorModal({ open, kind, namespace, name, data, loading, onClose, onSave }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="w-full max-w-xl rounded-xl border border-gray-300 dark:border-gray-800 bg-white dark:bg-gray-900 shadow-2xl flex flex-col max-h-[80vh]">
        {/* Header */}
            ...
            draft.map(([k, v], index) => (
              <div key={index} className="flex items-center gap-2">
                <input
                  ...
                  placeholder="Key"
                  className="flex-1 min-w-0 bg-gray-100 dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded px-2 py-1.5 text-xs text-gray-800 dark:text-gray-200 focus:outline-none focus:border-sky-600"
                />
                <input
                  value={v}
                  onChange={(e) => updateValue(index, e.target.value)}
                  placeholder="Value"
                  className="flex-[2] min-w-0 bg-gray-100 dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded px-2 py-1.5 text-xs text-gray-800 dark:text-gray-200 focus:outline-none focus:border-sky-600"
                />
                <button
